# This code produces plots of factors, X, and loadings, B, for all methods
# for one dataset from the simulation study
...
X_SSLB = X_SSLB[, union(c(1, 2, 6, 3, 4, 5, 8, 9, 10, 11, 7), 1:K_SSLB)]

B_SSLB = result_SSLB$B_found
B_SSLB = B_SSLB[, union(c(1, 2, 6, 3, 4, 5, 8, 9, 10, 11, 7), 1:K_SSLB)]
...
X_bicmix = result_bicmix$X_found
B_bicmix = result_bicmix$B_found

X_bicmix = X_bicmix[, union(c(1, 2, 10, 3), 1:ncol(X_bicmix))]
B_bicmix = B_bicmix[, union(c(1, 2, 10, 3), 1:ncol(X_bicmix))]
...
X_SSBiEM = X_SSBiEM[, union(c(7, 8, 5, 9:15, 6), 1:K)]
B_SSBiEM = B_SSBiEM[, union(c(7, 8, 5, 9:15, 6), 1:K)]


pdf("figures/X_ssbiem.pdf", width = 2, height = 4)

plot_matrix(X_SSBiEM, title = paste("X (SSBiEM, K = ", K_SSBiEM, ")", sep = ""), xlab = "K", ylab = "N")

dev.off()

pdf("figures/B_ssbiem.pdf", width = 2, height = 4)
...
X_SSBiEM = as.matrix(read.csv(paste(dir, "/matlab/X_K_large.txt", sep = ""), header = F))
B_SSBiEM = as.matrix(read.csv(paste(dir, "/matlab/B_K_large.txt", sep = ""), header = F))
X_SSBiEM_support = as.matrix(read.csv(paste(dir, "/matlab/X_support_K_large.txt", sep = ""), header = F))
B_SSBiEM_support = as.matrix(read.csv(paste(dir, "/matlab/B_support_K_large.txt", sep = ""), header = F))
...
X_SSBiEM = X_SSBiEM[, union(c(7, 8, 5, 9:15, 6), 1:K_SSBiEM)]
B_SSBiEM = B_SSBiEM[, union(c(7, 8, 5, 9:15, 6), 1:K_SSBiEM)]
