...
# directory for C code for BicMix (downloaded from 
# https://www.cs.princeton.edu/~bee/software/BicMix-Code-for-distribution.zip)
...
lambda0_out = matrix(NA, nrow = 3, ncol = nrep)
...
for (r in 1:nrep) {
  ...
  if (RUN_SSLB) {
    ...
    lambda0_out[1,r] = out$path$lambda0_tildes[length(lambda0_tildes)]
    ...
    lambda0_out[2,r] = out$path$lambda0_tildes[length(lambda0_tildes)]
    ...
    lambda0_out[3,r] = out$path$lambda0_tildes[length(lambda0_tildes)]
...
save(result, file = "results/sim_study_1.RData")
