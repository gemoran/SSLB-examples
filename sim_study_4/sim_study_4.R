# R script for Simulation 4 of SSLB paper
...
for (r in 1:nrep) {
  ...
  # generate data
  if (GEN_DATA) {
    get_data <- generate_dense_bic_poisson(n_f = N, n_l = G, n_bic = K, n_dense = 5, min_f = 5, max_f = 20,
  ...
  if (RUN_BICMIX) {
    ...
    X_bicmix <- as.matrix(X_bicmix[, keep])
    B_bicmix <- as.matrix(B_bicmix[, keep])
...
save(result, file = "results/sim_study_4.RData")
